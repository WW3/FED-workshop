function renderCarousel() {
  ...
  element.innerHTML = `
    <div class="slides" id="slds" aria-live="polite" aria-relevant="additions" aria-atomic="false"> 
    ${slides.items.map((item, i) => `
      
      ${item.index < sPos || item.index > ePos ? 
          `<div aria-hidden="true" id="slide${item.index}" class="slide"><a href="${item.url}" tabindex="-1">`
        : `<div id="slide${item.index}" class="slide"><a href="${item.url}">` 
      }
...
function updateDOM() {
  ...
  for (var i = 0; i < itms.length; i++) {
    if ((i+1) < sPos || (i+1) > ePos) {
      itms[i].setAttribute("aria-hidden", true);
      itms[i].getElementsByTagName("a")[0].setAttribute("tabindex", -1);
    } else {
      itms[i].removeAttribute("aria-hidden");
